...
export default function OutwardHistoryPage() {
  const { outwardHistory } = useSelector((state) => state.workspace);
  const navigate = useNavigate();
  const rows = useMemo(() => outwardHistory || [], [outwardHistory]);
  const pagination = usePagination(rows, 10);

  const openDetails = (record) => {
    const recordKey = record.id ?? record.code;
    if (!recordKey) return;
    navigate(`/workspace/outward/history/${encodeURIComponent(recordKey)}`, {
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500">All outward registers with line items for your assigned sites.</p>
        </div>
      </div>

      {rows.length === 0 && (
        <div className="table-card text-center text-sm text-slate-500">No outward records yet.</div>
      )}
